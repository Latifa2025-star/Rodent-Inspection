...
# 🐀 DOHMH Rodent Inspection Dataset: Phase 0: Data Loading
...
- **2,876,465 rows** → almost **3 million inspection records**, which is excellent for large-scale analysis and modeling.
- **25 columns** → each record includes many attributes such as location, date, inspection result, and geographic identifiers.
...
- **INSPECTION_TYPE** – “Initial” or “Re-inspection”
- **BOROUGH** – Bronx, Manhattan, Brooklyn, etc.
- **INSPECTION_DATE** – when the inspection happened
- **RESULT** – “Passed”, “Rat Activity”, etc.
- **LOCATION** – GPS coordinates (latitude/longitude)
- **CENSUS TRACT**, **COMMUNITY BOARD**, **NTA** – geographic units for mapping and spatial analysis
...
This step gives an overview of the dataset’s structure and missing values so I can plan the cleaning process.
...
- **10 object columns** → text fields (e.g., borough, street name, inspection type)
- **5 integer columns** → IDs, codes
- **10 float columns** → numeric values that include missing entries (e.g., census tract, community board)
...
- Type conversion (e.g., dates from string → datetime)
- Cleaning or filling missing values
- Encoding for modeling (categorical → numeric)
...
## 1. Row Counts (the “count” column)
The `count` values show how many non-missing values each numerical column contains.

Examples:
- `JOB_TICKET_OR_WORK_ORDER_ID` has **2.87 million non-missing values** → almost complete.
- `X_COORD`, `Y_COORD` have **only ~2.03 million values** → many missing coordinates.
- `LATITUDE`, `LONGITUDE` each have **~2.85 million values** → these are more complete and better for mapping.
...
- ZIP codes range mostly around **10000–11300**, matching NYC ZIPs.
...
- LONGITUDE ranges ~**–73.9°** (correct for NYC).
...
- `BORO_CODE`: 1–5 represent Manhattan, Bronx, Brooklyn, Queens, Staten Island.
- `BLOCK` ranges from 0 to 33,325 → valid for NYC parcel maps.
...
- **INSPECTION_TYPE**: 3 types (“Initial”, “BAIT”, “Compliance”). Helps analyze inspection workflows.
- **RESULT**: 5 main outcomes (e.g., “Passed”, “Rat Activity”). This will be important for prediction models.
...
Example:
- 2010-08-30 → Year = 2010, Month = 8  
- 2018-10-10 → Year = 2018, Month = 10  

This will allow me to analyze trends over time (yearly, monthly, seasonal patterns).  
The warning is normal — it just means pandas had to auto-detect the date format.
...
- The real analysis should focus on the modern inspection years (2010–present).
...
Here, I filter the dataset to keep only reasonable inspection years (2010–2024),
...
# Keep only realistic inspection years (2010–2024)
...
which is useful for visualizations and for shaping the project’s data story.
"""
...
- A dip in **2013–2015**  
- A major rise in **2016–2019**, with inspections peaking around 2018–2019  
- A sharp decline in **2020**, likely due to COVID-19  
- A recovery from **2021–2024**, returning to high levels
...
To check for seasonal patterns, I will count inspections for each month (January–December).
...
- **Winter months (Jan–Feb):** Moderate

This suggests that rodent inspections — and possibly rodent activity — tend to increase
...
plt.title("Rodent Inspections per Year (2010–2024)")
plt.xlabel("Year")
...
There is a steady increase from 2016 to 2019, reaching the highest inspection counts around 2018–2019.
In 2020, inspections drop sharply because of COVID-19 restrictions, then rise again starting in 2021 and return to high levels by 2022–2024.
...
To understand these patterns, we plot the total number of inspections for each month (Jan–Dec) across all years.  
...
This seasonal pattern confirms that rodent activity—and the city’s inspection responses—follow predictable yearly cycles.
...
Spring months, especially March, April, and May—have consistently higher inspection counts across most years, while winter months show lower activity.

There is a sharp drop across all months in 2020, reflecting the COVID-19 shutdown.
Inspections rise again in 2021–2024, returning to pre-pandemic levels.
...
This confirms a strong spatial pattern: rodent enforcement is most concentrated in the city’s denser and older housing areas.
...
"""**Rodent Inspections per Year (2010–2024)**"""
...
fig = px.line(
    ...
    title="Rodent Inspections per Year (2010–2024)"
)
...
"""Explanation (Rodent Inspections per Year – 2010 to 2024)

Rodent inspections were steady from 2010 to 2016, then rose sharply and reached their highest levels around 2017–2019. In 2020, there is a clear and dramatic drop due to COVID-19 shutdowns, when city operations slowed. After 2020, inspections quickly recovered and returned to pre-pandemic levels by 2022–2024.
...
"""Explanation (Rodent Inspections by Month – All Years Combined)
...
fig = px.pie(
    ...
    title="Inspection Outcomes Mix (2010–2024)"
)
...
# Cell D — Outcome Mix (Donut) — cleaned, grouped, and nicely labeled
...
**Top 20 Neighborhoods (NTA) — Horizontal Bar**
"""
...
This chart highlights the 20 NYC neighborhoods with the highest number of rodent inspections from 2010–2024. A few neighborhoods clearly stand out:
...
Overall, this ranking helps identify NYC’s rodent hotspots, guiding where targeted interventions, sanitation improvements, or predictive modeling efforts may have the strongest impact.

**Year × Month Heatmap (Interactive)**
"""
...
This map shows a sample of rodent inspections plotted across New York City. Each dot represents a single inspection, colored by the result. Large clusters of points appear in Manhattan, Brooklyn, and the Bronx, showing where inspections—and rodent activity—are most concentrated. Areas with more green and red dots indicate neighborhoods with more “Rat Activity” findings or bait applications. Overall, the map highlights the geographic hotspots where rodent issues are most common.
...
fig = px.bar(imp, x='importance', y='feature', orientation='h',
             ...
             title="Random Forest — Top Feature Importances (Preview)")
